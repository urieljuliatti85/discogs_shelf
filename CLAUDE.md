...
`DiscogsSyncJob` runs `Sync` in the background (Active Job `:async` in development, Solid Queue in production). `POST /api/sync` refuses to start a second run while `SyncRun.running?`; that guard only looks at runs created in the last hour, so a run killed mid-flight stops blocking after an hour instead of wedging sync forever. The check-and-create runs inside `Api::SyncController#start_run`'s `SyncRun.transaction do ... end` — a plain (non-isolated) ActiveRecord transaction opens SQLite with `BEGIN IMMEDIATE`, taking the write lock up front, so two near-simultaneous requests can't both see "not running" before either has inserted its row. The frontend `useSync` hook polls `GET /api/sync` every 1.5s only while a run is in flight, then bumps `dataVersion` in `AppContext` so every mounted list refetches.
...
**Rate limiting** — every endpoint that can trigger a Discogs request is capped with Rails' built-in `rate_limit` (`ActionController::RateLimiting`), keyed by IP: `POST /api/sync` at 5/10min (`SyncRun.running?` already blocks a *concurrent* second run, this blocks repeated *attempts*), and `GET /api/releases/:id` + `marketplace` at 60/min (each miss on the 30-day details cache is a Discogs request). Both share `Api::BaseController::RATE_LIMIT_STORE`, which is `Rails.cache` (solid_cache in production, so the limit is shared across Puma workers) except in test, where `cache_store` is `:null_store` and would make the limit silently inert — test gets its own process-wide `MemoryStore` instead. `test/test_helper.rb` clears that store before every test.
